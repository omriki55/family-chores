...
export default function HomeScreen({S,app}){
  const{user,isP,tasks,completions,cKey,wk,xp,streaks,goals,challenges,messages,swaps,
    ...
    locations,setLocations,save,flash,
    getNextReminder}=app;
    ...
    <>
      ...
      {activeReminder&&!isP&&(
        <div style={{background:"linear-gradient(135deg,#f59e0b20,#f59e0b10)",border:"1px solid #f59e0b40",borderRadius:12,padding:12,marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:700,color:"#f59e0b"}}>{t("home.reminder",{label:activeReminder.label})}</div>
            <div style={{fontSize:11,color:"#fbbf24"}}>{t("home.pendingTasks",{count:tasks.filter(tk=>app.isTaskForChild(tk,user,today)&&!tk.bonus&&!completions[cKey(tk.id,user,today)]?.done).length})}</div>
          </div>
      ...
      <div style={{textAlign:"center",marginBottom:14}}>
        <div style={{fontSize:16,fontWeight:800,color:"var(--text)"}}>
          {getHour()<12?t("greet.morning"):getHour()<17?t("greet.afternoon"):getHour()<21?t("greet.evening"):t("greet.night")}, {me.name}!
        </div>
        <div style={{fontSize:11,color:"var(--textSec)"}}>{t("day.prefix")} {DAYS[today]}</div>
      </div>
      ...
      {installReady&&<div style={{background:"linear-gradient(135deg,#6366f120,#8b5cf620)",border:"1px solid #6366f160",borderRadius:12,padding:12,marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:"#6366f1"}}>{t("home.installPrompt")}</div>
          <div style={{fontSize:9,color:"var(--textTer)"}}>{t("home.installDesc")}</div></div>
        <button onClick={handleInstall} style={{padding:"6px 12px",background:"#6366f1",border:"none",borderRadius:8,color:"#fff",fontSize:10,fontWeight:700,cursor:"pointer"}}>{t("home.install")}</button>
      ...
      {!isP&&"Notification" in window&&Notification.permission==="default"&&<div style={{background:"#fff7ed",border:"1px solid #f59e0b40",borderRadius:12,padding:10,marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{flex:1,fontSize:10,color:"#92400e"}}>{t("home.enableNotifications")}</div>
        <button onClick={()=>Notification.requestPermission()} style={{padding:"4px 10px",background:"#f59e0b",border:"none",borderRadius:6,color:"#fff",fontSize:9,fontWeight:700,cursor:"pointer"}}>{t("home.enable")}</button>
      </div>}
      ...
      {(()=>{
        ...
        return(
          <div style={{background:"var(--card)",borderRadius:14,padding:12,marginBottom:10,border:"1px solid var(--border)"}}>
            <div style={{fontSize:12,fontWeight:800,color:"var(--text)",marginBottom:8}}>{t("home.whereNow")}</div>
      ...
      {!isP&&(()=>{
        ...
        const todayTasks=tasks.filter(tk=>app.isTaskForChild(tk,user,today)&&!tk.bonus);
        const todayDone=todayTasks.filter(tk=>completions[cKey(tk.id,user,today)]?.done).length;
        return(
          <>
            {/* Level & XP card */}
            <div style={{background:`linear-gradient(135deg,${FAMILY[user]?.color||'#6366f1'}15,${FAMILY[user]?.color||'#6366f1'}08)`,borderRadius:14,padding:14,marginBottom:10,border:"1px solid #4f46e540"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                <div style={{fontSize:36,cursor:"pointer"}} onClick={()=>{app.setProfileChild&&app.setProfileChild(user);setScreen("profile");}}>{app.avatars?.[user]?.value||lv.emoji}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:800,color:"var(--text)"}}>{lv.name}</div>
                  <div style={{fontSize:10,color:"#6366f1"}}>{xp[user]||0} XP {nxt?`• ${t("home.xpMore",{amount:nxt.min-(xp[user]||0),name:nxt.name})}`:""}</div>
                ...
                {(streaks[user]||0)>0&&<div style={{textAlign:"center",background:"#f59e0b20",borderRadius:10,padding:"6px 10px"}}>
                  ...
                  <div style={{fontSize:8,color:"#fbbf24"}}>{t("home.streak")}</div>
                </div>}
              </div>
            </div>
            ...
            {/* Next reminder */}
            {(()=>{const nr=getNextReminder&&getNextReminder(user);if(!nr)return null;return(
              <div style={{background:"#ede9fe",border:"1px solid #c4b5fd",borderRadius:10,padding:"8px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:14}}>⏰</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,fontWeight:600,color:"#6d28d9"}}>{nr.label||t("home.nextReminder")} — {nr.time}</div>
                </div>
              </div>
            );})()}
            ...
            <div style={{background:"var(--card)",borderRadius:14,padding:14,marginBottom:10,border:"1px solid var(--border)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>{t("home.todayProgress")}</span>
              ...
              {todayTasks.filter(tk=>!completions[cKey(tk.id,user,today)]?.done).slice(0,4).map(tk=>(
                <div key={tk.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:"1px solid var(--border)"}}>
                  <span style={{fontSize:14}}>{tk.icon}</span>
                  <span style={{fontSize:12,color:"var(--textQuat)",flex:1}}>{tk.title}</span>
                  <button onClick={()=>setDoneConfirm({taskId:tk.id,childId:user,day:today})} style={{...S.doneBtn,width:28,height:28,fontSize:13,borderRadius:7}}>✓</button>
                </div>
              ))}
              {todayDone===todayTasks.length&&todayTasks.length>0&&(
                <div style={{textAlign:"center",padding:8}}><span style={{fontSize:13,color:"#10b981",fontWeight:700}}>{t("home.allTasksDone")}</span></div>
              )}
            ...
            {me.weeklyPay>0&&(
              <div style={{background:"linear-gradient(135deg,#ecfdf5,#d1fae5)",borderRadius:14,padding:14,marginBottom:10,border:"1px solid #10b98140"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><div style={{fontSize:12,color:"#059669"}}>{t("home.earnedThisWeek")}</div>
            ...
            {(()=>{const myBadges=earnedBadges[user]||[];if(myBadges.length===0)return null;return(
              <div style={{background:"var(--card)",borderRadius:14,padding:12,marginBottom:10,border:"1px solid #f59e0b40"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  <span style={{fontSize:12,fontWeight:700,color:"var(--text)"}}>{t("home.badges")}</span>
                  <button onClick={()=>setScreen("badges")} style={{background:"none",border:"none",color:"#6366f1",fontSize:10,cursor:"pointer"}}>{t("action.viewAll")}</button>
                </div>
      ...
      {isP&&(
        <>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      <span style={{fontSize:13,fontWeight:700,color:m.color,cursor:"pointer"}} onClick={()=>{app.setProfileChild&&app.setProfileChild(cid);setScreen("profile");}}>{app.avatars?.[cid]?.value||m.emoji} {m.name}</span>
                      <span style={{fontSize:10}}>{lv.emoji}</span>
                      {(streaks[cid]||0)>0&&<span style={{fontSize:9,color:"#f59e0b"}}>🔥{streaks[cid]}</span>}
                    </div>
                    <div style={{fontSize:10,color:"var(--textTer)"}}>{t("home.todayWeekPct",{todayPct,weekPct:st.pct})} {m.weeklyPay>0?`• ${st.earned}₪`:""}  </div>
                  </div>
          ...
          <button onClick={()=>setExamModal(true)} style={{width:"100%",padding:10,background:"linear-gradient(135deg,#6366f120,#6366f110)",border:"1px solid #6366f140",borderRadius:12,color:"#6366f1",fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:10}}>{t("home.reportExam")}</button>
          {(()=>{let cnt=0;tasks.forEach(tk=>tk.assignedTo.forEach(c=>{for(let d=0;d<7;d++){if(completions[cKey(tk.id,c,d)]?.done&&!completions[cKey(tk.id,c,d)]?.approved)cnt++;}}));
          ...
          {swaps.filter(s=>s.status==="pending").length>0&&(
            <div style={{background:"var(--card)",borderRadius:12,padding:12,marginBottom:10,border:"1px solid #8b5cf640"}}>
              ...
              {swaps.filter(s=>s.status==="pending").map(s=>{const tk=tasks.find(x=>x.id===s.taskId);return(
                <div key={s.id} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 0",borderBottom:"1px solid var(--border)"}}>
                  <span style={{fontSize:12}}>{tk?.icon}</span>
                  <span style={{fontSize:11,color:"var(--textQuat)",flex:1}}>{FAMILY[s.from]?.name} → {FAMILY[s.to]?.name}: {tk?.title}</span>
      ...
      <div style={{background:"linear-gradient(135deg,#ede9fe,#ddd6fe)",borderRadius:14,padding:14,border:"1px solid #4f46e580",marginBottom:10}}>
        <div style={{fontSize:13,fontWeight:800,color:"var(--text)",marginBottom:8}}>{t("home.familyGoal")}</div>
          ...
          return(
            <div key={g.id} style={{background:"rgba(99,102,241,0.06)",borderRadius:10,padding:10,marginBottom:6}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                ...
                {achieved&&<span style={{fontSize:11,color:"#10b981",fontWeight:700}}>{t("home.achieved")}</span>}
              </div>
              ...
              <div style={{fontSize:10,color:"#7c3aed"}}>{t("home.prize",{reward:g.reward})} • {fp}%/{g.target}%</div>
            </div>
          );
        })}
      </div>
      ...
      {(()=>{const wkCh=challenges.filter(c=>c.week===wk);if(!wkCh.length)return null;
        return<div style={{background:"linear-gradient(135deg,#fef3c7,#fefce8)",borderRadius:14,padding:14,border:"1px solid #f59e0b80",marginBottom:10}}>
          <div style={{fontSize:13,fontWeight:800,color:"var(--text)",marginBottom:8}}>{t("home.weekChallenges")}</div>
